...
func set_state(_state:State) -> void:
	...
	current_page_instance.initial_state = _state.duplicate()
	current_page_instance.history.clear()
	current_page_instance.history_raw.clear()
	if _state.get_turn() == 1:
		current_page_instance.history.push_back("-")
		current_page_instance.history_raw.push_back(-1)
	update_table()

func push_move(move:int) -> void:
	if current_page_instance.history.size() >= 120:
		return
	current_page_instance.history_raw.push_back(move)
...
func update_table() -> void:
	$history/chessboard_flat.set_state(current_page_instance.state)
	...
	for i:int in range(current_page_instance.history.size()):
		if i % 2 == 0:
			get_node("history/white/label_%d" % (i / 2 + 1)).text = current_page_instance.history[i]
		else:
			get_node("history/black/label_%d" % (i / 2 + 1)).text = current_page_instance.history[i]
